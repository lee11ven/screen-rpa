...
import type { EditorNode, OnErrorPolicy, RuntimeWorkflow, WfNodeType } from '../../dsl/types'
import { editorWorkflowToLfData, parseWfType, type LfRawGraph } from '../../logicflow/adapters'
...
import LoopSubgraphEditorDialog from './LoopSubgraphEditorDialog.vue'
...
const loopSubgraphDialogVisible = ref(false)
const loopSubgraphEditorNodeId = ref<string | null>(null)

function openLoopSubgraphEditor() {
  const n = selectedNode.value
  if (!n || n.type !== 'loop-container') return
  loopSubgraphEditorNodeId.value = n.id
  loopSubgraphDialogVisible.value = true
}
...
function rawGraph(): LfRawGraph {
  return lf!.getGraphRawData() as unknown as LfRawGraph
}

function syncFromLf(recordHistory: boolean) {
  if (!lf) return
  store.applyGraphFromLf(rawGraph(), { recordHistory })
}

function ensureSingleStart() {
  if (!lf) return
  const raw = rawGraph()
  const starts = raw.nodes.filter((n) => parseWfType(n) === 'start')
  if (starts.length > 1) {
    const drop = starts[starts.length - 1]!
    lf.deleteNode(drop.id)
    ElMessage.warning('仅允许一个开始节点')
  }
}
...
function bindEvents() {
  if (!lf) return
  lf.on('node:dragend', () => syncFromLf(true))
  lf.on('node:add', () => {
    ensureSingleStart()
    syncFromLf(true)
  })
  lf.on('node:delete', () => syncFromLf(true))
  lf.on(
    'edge:add',
    (e: unknown) =>
      onEdgeAdd(
  ...
  lf.on('edge:delete', () => syncFromLf(true))
  lf.on('node:click', ({ data }) => {
    store.selectedNodeId = data?.id ?? null
...
function deleteCurrentSelection() {
  ...
  if (store.selectedNodeId) {
    lf.deleteNode(store.selectedNodeId)
    store.selectedNodeId = null
    return
  }
...
function onKeydown(e: KeyboardEvent) {
  if (e.key === 'Escape' && lowcodeFullscreen.value) {
    e.preventDefault()
    lowcodeFullscreen.value = false
    return
  }
...
function renderWorkflow() {
  if (!lf || !store.currentWorkflow) return
  const data = editorWorkflowToLfData(store.currentWorkflow)
  lf.render(data as unknown as Parameters<LogicFlow['render']>[0])
  removeModificationOverlay(containerRef.value)
}

watch(
  () => store.canvasGeneration,
  () => {
    void nextTick(() => renderWorkflow())
  },
)
...
const selectedNode = computed(() => {
  ...
  if (!id || !store.currentWorkflow) return null
  return store.currentWorkflow.nodes.find((n) => n.id === id) ?? null
})
...
function isSelfWorkflowSelection(workflowId: string): boolean {
  ...
  const currentWorkflowId = String(store.currentWorkflow?.workflow_id || '').trim()
...
function applyNodeConfig() {
  ...
  store.updateSelectedNodeConfig({ ...base, config })
}
...
async function loadLowcodeTestCtxFromGlobalConfig(): Promise<Record<string, unknown>> {
  const workflowId = store.currentWorkflow?.workflow_id
...
async function runLowcodeFunctionTest() {
  if (selectedNode.value?.type !== 'lowcode_function') return
  clearLowcodeTestResult()
...
function applyEdgeConfig() {
  ...
  if (!edgeModel) return
  const isIfEdge = selectedEdge.value?.sourceType === 'if'
  ...
  <div ref="canvasWrapRef" class="canvas-wrap">
    ...
    <div v-if="selectedNode" class="node-config-panel">
      <div class="node-config-head">
        ...
        <el-button text @click="closeConfigPanel">关闭</el-button>
      </div>
      <el-form label-position="top" @submit.prevent>
        ...
        <div
          v-if="
            ...
            selectedNode.type === 'loop-container'
          "
          class="config-group"
        >
          ...
          <template v-if="selectedNode.type === 'subflow'">
            <el-form-item label="子流程">
              <el-select v-model="form.subflowId" filterable placeholder="选择 workflow" @change="onSubflowChange">
                <el-option
                  v-for="w in wfList.items"
                  :key="w.workflow_id"
                  :label="`${w.name} (${w.workflow_id})${isSelfWorkflowSelection(w.workflow_id) ? ' - 当前流程，禁选' : ''}`"
                  :value="w.workflow_id"
                  :disabled="isSelfWorkflowSelection(w.workflow_id)"
                />
              </el-select>
            </el-form-item>
          </template>
          ...
          <template v-if="selectedNode.type === 'loop-container'">
            <el-form-item>
              <el-button type="primary" plain @click="openLoopSubgraphEditor">编辑循环体内流程</el-button>
            </el-form-item>
          </template>
        </div>
    ...
    <div v-else-if="selectedEdge" class="node-config-panel">
      <div class="node-config-head">
        ...
        <el-button text @click="selectedEdgeId = null">关闭</el-button>
      </div>
      <el-form label-position="top" @submit.prevent>
        <div class="config-group">
          <div class="config-group-title">基础信息</div>
    ...
    <div
      ...
      @contextmenu.prevent
    >
      <el-button text type="danger" @click="deleteByContextMenu">删除</el-button>
    </div>
    <LoopSubgraphEditorDialog v-model="loopSubgraphDialogVisible" :loop-node-id="loopSubgraphEditorNodeId" />
  </div>
</template>
...
/* 主画布是 svg.lf-canvas-overlay；网格在 DOM 中在其后。z-index 对 static 的 svg 无效，且 .lf-graph svg 会误选 .lf-grid 内 svg */
...
/* 彻底移除 LogicFlow modification-overlay */
